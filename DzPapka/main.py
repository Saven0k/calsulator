...
def work():
    ...
    while data != 0:
        if data == 1:
            ...
            item = int(input("Введите номер записи: "))
            log.wr2(f"Номер записи {item}")

            data_recording.write(second_name, name , phone_number , info , item , phonebook )
            ...
            dd = int(input("Введите номер записи, которую ищите: "))
            search.search_data(dd , phonebook)
            print('                      \n                            \n                 ')
            work()
